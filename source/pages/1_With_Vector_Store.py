...
def refresh_metrics():
   with st.sidebar:
    ...
    if 'bleu' in st.session_state:
      ...
      if 'delta' in st.session_state['bleu']: st.metric(label="BLEU", value=str(round(bleu['score'], 2)), delta=str(round(bleu['delta'], 2)))
    ...
    if 'bert_score' in st.session_state:
      ...
      if 'bert_delta' in bert_score:
          st.metric(label="BERTScore F1", 
                    value=str(round(bert_score['f1'] * 100, 2)), 
                    delta=str(round(bert_score['bert_delta'] * 100, 2)))
      else:
          st.metric(label="BERTScore F1", 
                    value=str(round(bert_score['f1'] * 100, 2)))
    # METEOR
    if 'meteor' in st.session_state:
      ...
      if 'meteor_delta' in meteor:
          st.metric(label="METEOR", 
                    value=str(round(meteor['score'] * 100, 2)), 
                    delta=str(round(meteor['meteor_delta'] * 100, 2)))
      else:
          st.metric(label="METEOR", 
                    value=str(round(meteor['score'] * 100, 2)))
    # ChrF
    if 'chrf' in st.session_state:
      ...
      if 'chrf_delta' in chrf:
          st.metric(label="ChrF", 
                    value=str(round(chrf['score'] * 100, 2)), 
                    delta=str(round(chrf['chrf_delta'] * 100, 2)))
      else:
          st.metric(label="ChrF", 
                    value=str(round(chrf['score'] * 100, 2)))
