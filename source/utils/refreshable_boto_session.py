...
from .config import get_app_role_arn, get_session_ttl, get_region

"""
Creates a static singleton of RefreshableBotoSession based on region_name, role_arn, session_name, and session_ttl, caches it for reuse and returns it
"""
def get_refreshable_boto_session():
    """
    Creates a static singleton of RefreshableBotoSession based on region_name, role_arn, session_name, and session_ttl, caches it for reuse and returns it
    """
    if not hasattr(get_refreshable_boto_session, "refreshable_boto_session"):
        sts_arn = get_app_role_arn()
        session_name = "LLMTranslationPlayground-Session"
        session_ttl = get_session_ttl()
        region_name = get_region()

        _refreshable_boto_session = RefreshableBotoSession(region_name=region_name, sts_arn=sts_arn, session_name=session_name, session_ttl=session_ttl)
        get_refreshable_boto_session.refreshable_boto_session = _refreshable_boto_session.refreshable_session()
    return get_refreshable_boto_session.refreshable_boto_session
...
class RefreshableBotoSession:
    ...
    def __get_session_credentials(self):
        ...
        if self.sts_arn:
            ...
            print("Successfully assumed role arn: ", self.sts_arn)
        else:
